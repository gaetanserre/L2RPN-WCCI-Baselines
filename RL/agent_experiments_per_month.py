...
def cli():
    ...
    parser.add_argument("--months_to_train",
                        nargs='+',
                        help="You can make a selection among '01' for january until '12' for december")
...
if __name__ == "__main__":
    ...
    months_to_train = copy.deepcopy(args.months_to_train)
    print("months_to_train:", months_to_train)
    n_months = len(months_to_train)

    # now do the loop to train the agents
    for i_train in range(nb_train):
        ...
        #     print("_order:"env_train.chronics_handler.real_data._order)
        i_month = i_train % n_months
        pattern_month = "\d{4}-" + months_to_train[i_month] + "-\d{2}\w*"
        print("pattern_month:", pattern_month)
        def filter_chronics(x, pattern = pattern_month):
            res = re.search(pattern, x) is not None
            return res
        env_train.chronics_handler.real_data.set_filter(filter_chronics)
        env_train.chronics_handler.real_data.reset()
        print("50 first chronics' names:", env_train.chronics_handler.real_data.available_chronics()[:20])
        ...
        agent_name = f"{args.agent_name}_{datetime.datetime.now():%Y%m%d_%H%M%S}_month{months_to_train[i_month]}"
